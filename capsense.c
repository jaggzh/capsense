...
void set_recent_limits(cp_st *cp) {
	for (int i=CP_COL_DATASTART; i<CP_COLCNT; i++) {
		float v = cp->cols[i];
		/* printf("  Limit [%d] %f\n", i, v); */
		if (v < cp->rmin) cp->rmin = v - 1;
		if (v > cp->rmax) cp->rmax = v + 1;
	}
}
	/* printf("Min:%f Max:%f\n", rmin, rmax); */
...
char fail_safety_points(int i, int ms, int y,
...
void detect_pressevents(cp_st *cp) {
	...
	if (bst == BST_OPEN) { // Default state: Open == not a cap-sense press
		if (startdelta > noiserange*close_thresh) {
			st = sample_count;
			stref = startyref;
			bst = BST_CLOSED_DEBOUNCE;
			//mark_start_try(plot=plot, ms=d['millis'][i], y=starty);
		}
	} else if (bst == BST_CLOSED_DEBOUNCE) {
	...
	} else if (bst == BST_CLOSED) {
		if (fail_safety_points(sample_count, ms, starty,
				noiserange,
			...
			bst = BST_OPEN_DEBOUNCE;
		}
	} else if (bst == BST_OPEN_DEBOUNCE) {
